...
### Spring Tool Suite
I’ve used spring tool suite for quite a long time. It’s more stable than the general eclipse distribution, and is easy to install because it already contains the plugins needed for nGrinder development.

Most of all, the biggest benefit of Spring Tool Suite is “Java Agent-based reloading.”. Usually, java developers spend a great deal of time restarting web server(tomcat) whenever code is changed. There is good auto-reloading support in JRebel, but it’s not free. STS’s “Java Agent-based reloading” provides this feature for developers who need a low-cost solution. It dynamically reloads the changes so that you can save time. You can download STS from the following link:
- http://www.springsource.org/downloads/sts
After installing it, you have to install Spring version of tomcat called VMware vFabric tc server developer edition. This is free, and can be downloaded from the following link:
- https://my.vmware.com/web/vmware/evalcenter?p=tcserver-dev&lp=default&rct=j&q=&esrc="s&source=web&cd=2&ved=0CDMQFjAB&url=http://www.vmware.com/go/vfabric-tcserver-dev&ei=vx8IUOi5LM3JrAeC7I3TAg&usg=AFQjCNGFBrvNmv9APJrAsI7huFhW8nHgWA&sig2=AOtPIzfpYujaif12JCZHww

When you “Run on Server”, you have to choose the VMware vFabric server. In the configuration page in the Server view of STS, double click on the VMware vFabric server.

![](http://www.cubrid.org/files/attach/images/379199/174/395/image_thumb.png)

Then just select "Enable Java Agent-based reloading."

![](http://www.cubrid.org/files/attach/images/379199/174/395/image_thumb_1.png)

Even though it’s described as an experimental version, nGrinder mostly works well in this configuration. When you change annotations or xml files, and a complete spring rescan is requried, you will have to restart the server. But if you are just changing class method implementation, simply saving the java class is enough to reflect the changes.
